...
exports.nombreUtilisateurs = async (req, res) => {
    try {
        const [result] = await db.query('SELECT COUNT(*) AS totalUtilisateurs FROM users WHERE admin = 0');
        res.status(200).json({ totalUtilisateurs: result[0].totalUtilisateurs });
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Une erreur s\'est produite lors du comptage des utilisateurs' });
    }
};
